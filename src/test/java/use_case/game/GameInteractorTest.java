...
class GameInteractorTest {
    ...
    @Test
    void testExecuteDoorExitDoor() throws InvocationTargetException, InterruptedException {
        Scene sceneTable = new Scene("Scene Table", new ArrayList<>(), "table.png");
        Scene sceneClassroom = new Scene("Scene Classroom", new ArrayList<>(), "classroom.png");
        Scene sceneStairs = new Scene("Scene Stairs", new ArrayList<>(), "stairs.png");
        Scene sceneExit = new Scene("Scene Exit", new ArrayList<>(), "exit.png");
        ...
        Player player = new Player();
        Collectibles key = new Collectibles("Key Exit", 0, 0, "key.png");
        player.addToInventory(key);
        dao.setPlayer(player);

        assertFalse(dao.isDoorUnlocked("Door Exit"));
        assertTrue(dao.getPlayer().hasItemNamed("Key Exit"));

        ClickableObject doorExit = new ClickableObject("Door Exit", 100, 100, "door.png");
        GameInputData inputData = new GameInputData(doorExit);
        ...
        assertTrue(dao.isDoorUnlocked("Door Exit"));
        assertFalse(dao.getPlayer().hasItemNamed("Key Exit"));
        assertEquals(sceneExit, dao.getCurrentScene());
    }
